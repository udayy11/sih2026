  ...
  const handleUpdateRole = async (user: User, newRole: string) => {
    try {
      ...
      if (res.ok) {
        ...
        setSuccessMessage(`Role for ${user.name} successfully updated to ${newRole}`);
          ...
          <table className="w-full text-xs text-left">
            <thead className="text-slate-500 uppercase bg-slate-50 border-b border-slate-200 font-bold">
              <tr>
                ...
                <th className="px-6 py-3.5 text-right">Edit RBAC Role</th>
              </tr>
            </thead>
              ...
              {filteredUsers.map((user) => (
                <tr key={user.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/60 transition-colors">
                  ...
                  <td className="px-6 py-4 text-center">
                    <span className="px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 dark:bg-emerald-900/50 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700">
                      ● {user.status}
                    </span>
                  </td>
