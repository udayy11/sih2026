...
export function transformMospiRecord(raw: RawMospiProject, index: number): InfrastructureProject {
  ...
  let shortIssuesSummary = '';
  if (delayMonths > 12 && costOverrunPercent > 15) {
    shortIssuesSummary = `Critical delay (+${delayMonths}m), Cost escalation (+${costOverrunPercent.toFixed(1)}%) & land dispute`;
  } else if (delayMonths > 12) {
    shortIssuesSummary = `Significant schedule delay (+${delayMonths} mos), pending forest/land clearance`;
  } else if (costOverrunPercent > 10) {
    shortIssuesSummary = `Budget overrun (+₹${costOverrunAmount.toLocaleString('en-IN')} Cr / +${costOverrunPercent.toFixed(1)}%) due to material inflation`;
  } else if (progressExpenditureDivergence < -15) {
    shortIssuesSummary = `High expenditure burn (${financialProgress}%) with slow physical output (${safePhysicalProgress}%)`;
  } else if (delayMonths > 0) {
    shortIssuesSummary = `Schedule lag (+${delayMonths} mos) due to utility shifting & seasonal rains`;
  } else {
    shortIssuesSummary = `On schedule; normal execution velocity`;
  }
  ...
  return {
    ...
    shortIssuesSummary: shortIssuesSummary,
    evidence: [
