...
// Update user role (RBAC edit)
authRouter.post('/update-role', (req, res) => {
  const { userId, newRole } = req.body;
  if (!userId || !newRole) {
    return res.status(400).json({ error: 'userId and newRole are required' });
  }

  const user = users.find((u) => u.id === userId || u.username === userId.toLowerCase());
  if (!user) {
    return res.status(404).json({ error: 'User not found in database' });
  }

  let roleEnum: Role = 'PROJECT_OFFICER';
  if (newRole.toLowerCase().includes('admin')) roleEnum = 'ADMIN';
  else if (newRole.toLowerCase().includes('tracker') || newRole.toLowerCase().includes('mospi')) roleEnum = 'MOSPI_OFFICER';
  else if (newRole.toLowerCase().includes('ministry')) roleEnum = 'MINISTRY_OFFICER';
  else roleEnum = 'PROJECT_OFFICER';
